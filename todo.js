...
function renderTasks(filteredTasks = tasks) {
    ...
    filteredTasks.forEach((task, index) => {
        ...
        if (task.deadline && task.text) {
            addEvent(task.deadline, task.text);
        }
...
function deleteEvent(date, eventName) {
    ...
    const taskToDelete = tasks.find(task => task.text === eventName && task.deadline === date);
    if (taskToDelete) {
        const taskIndex = tasks.indexOf(taskToDelete);
        if (taskIndex !== -1) {
            deleteTask(taskIndex, date);
        }
    }
}
